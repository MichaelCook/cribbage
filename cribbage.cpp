...
constexpr size_t num_suits = sizeof(suit_chars) - 1;
constexpr size_t num_ranks = sizeof(rank_chars) - 1;
...
public:
  ...
  constexpr Rank rank() const noexcept {
    assert(std::popcount(card_) == 1);
    uint64_t n = std::popcount(card_ - 1);
    return n % 16;
  }
